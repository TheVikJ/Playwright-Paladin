...
app.use(cors({
    origin: prodEnv === 'development' ? 'http://localhost:3000' : 'https://playwrightpaladin.co',
    credentials: true
}))
...
app.post('/api/auth/refresh', (req, res) => {
    ...
    jwt.verify(refreshToken, jwtRefreshToken, async (err, phone) => {
        if (!err) {
            const accessToken = jwt.sign({data: phone}, jwtAuthToken, {expiresIn: '1d'})
...
app.post('/api/auth/verifyOTP', (req, res) => {
    ...
    const data = `${phone}.${otp}.${expires}`
    const calculatedHash = crypto.createHmac('sha256', smsKey).update(data).digest('hex')
    ...
    const accessToken = jwt.sign({data: phone}, jwtAuthToken, {expiresIn: '1d'})
    const refreshToken = jwt.sign({data: phone}, jwtRefreshToken, {expiresIn: '1y'})
